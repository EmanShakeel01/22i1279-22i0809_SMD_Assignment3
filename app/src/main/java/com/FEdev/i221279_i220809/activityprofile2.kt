...
class activityprofile2 : AppCompatActivity() {
    ...
    private fun displayUserProfile(user: com.FEdev.i221279_i220809.models.UserProfile) {
        ...
        setupOnlineStatusListener(user.user_id)
    }
    ...
    private fun setupOnlineStatusListener(userId: Int) {
        // Cancel previous listener if any
        statusListenerJob?.cancel()

        // Start listening to status
        statusListenerJob = OnlineStatusManager.listenToUserStatus(userId) { isOnline, lastSeen ->
            runOnUiThread {
                statusText.visibility = View.VISIBLE
